...
const SmallLetter = () => {
    ...
    const handleToggleLetter = () => {
        ...
        if (isOpen) {
            ...
            letterElement.classList.add('small-SAMit-letter--close');
            setIsOpen(false);

            // 2. Remove the closing class after the animation duration (600ms)
            setTimeout(() => {
                letterElement.classList.remove('small-SAMit-letter--close');
            }, 600);
            ...
            letterElement.classList.remove('small-SAMit-letter--close');
            // 2. Set state to open (which adds 'small-SAMit-letter--open')
            setIsOpen(true);
        }
    };
    ...
    const handleCloseLetter = () => {
        ...
        letterElement.classList.remove('small-SAMit-letter--open');
        letterElement.classList.add('small-SAMit-letter--close');
    ...
    const letterStateClass = isOpen ? 'small-SAMit-letter--open' : '';

    return (
        <>
            <div
                className={`small-SAMit-letter ${letterStateClass}`}
                ref={letterRef} // Attach the ref here
            >
                <div
                    className="small-SAMit-envelope"
                    onClick={handleToggleLetter} // Attach the toggle handler
                >
                    <div className="small-SAMit-envelope-flap"></div>
                    <div className="small-SAMit-envelope-paper"></div>
                    <div className="small-SAMit-envelope-detail"></div>
                </div>

                <div className="small-SAMit-paper">
    <div className="small-SAMit-paper-content">
        <div
            className="small-SAMit-paper-close"
            onClick={handleCloseLetter} // Attach the close handler
        >
            x
        </div>
        <p>dear tahuuu😘<br /><br />    Thank you for coming into my life. Things wouldn't be the same without you. We may fight a bit too much, but please never give up on us—hold onto me no matter what. I love you so much, and Happy Birthday again😘
  
  <br /><br />SAMit</p>
    </div>

    {/* --- NEW SVG DIARY PAGE DECORATION --- */}
    <svg className="SAMit-diary-deco" viewBox="0 0 100 100" preserveAspectRatio="none">
        {/* Left 'binding' line */}
